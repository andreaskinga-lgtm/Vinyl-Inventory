// PROTOTYPE — throwaway. Shared *data* helpers only.
// Deliberately no shared layout/chrome: each variant owns its own structure.
...
// Divider-card key for a record, given the active sort. This is the label that
// would be printed on the tab sticking up out of the crate.
...
// Real (read-only) tracklist fetch — this is the thing that makes the mobile
// detail view jump today, so the prototype has to reproduce it for real.
...
export function requestedVariant() {
  ...
  return VARIANTS.some((v) => v.key === key) ? key : null;
}
